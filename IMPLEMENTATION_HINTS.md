...
| En producción, `pool timeout: failed to retrieve a connection from pool` **después de cada reinicio de MySQL**, mientras el cliente `mysql` de la consola entra sin problema con el mismo usuario | MySQL 8 autentica con `caching_sha2_password`. Sobre TCP sin TLS, la primera autenticación posterior a cada arranque del servidor necesita su clave pública RSA, y el conector de MariaDB no se la pide salvo que se lo autoricen. La consola no lo sufre porque va por el socket Unix, que no necesita RSA — y de paso llena la caché del servidor, así que la aplicación vuelve a andar sola y el síntoma parece intermitente | `lib/db/urlConexion.ts` agrega `allowPublicKeyRetrieval=true` **solo contra loopback**, que es donde el intercambio no se puede interceptar. Con la base en otra máquina hay que usar TLS o `cachingRsaPublicKey`. Diagnóstico: `scripts/diag-db.mjs` |
